...
### 2. `multi_key_gemini_ranker.py` (574 строки)
...
python reset_ranking.py --stats
```

### 11. `reset_video_data.py` (240 строк)
**Назначение**: Управление данными видео (транскрипт и summary)

**Основные функции**:
- `reset_video_data(video_id, transcript_only, summary_only)` - сброс данных видео
- `reset_all_video_data()` - массовый сброс данных всех видео
- `show_video_stats()` - статистика видео
- `main()` - CLI интерфейс

**Опции**:
- `--transcript-only` - сброс только транскрипта
- `--summary-only` - сброс только summary
- `--all` - сброс данных всех видео
